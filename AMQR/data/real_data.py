...
def load_pems_traffic_and_locations(h5_path, csv_path, num_nodes=20):
    """
    Load PEMS-BAY traffic speed data and sensor spatial coordinates (latitude and longitude).
    Returns: Y_flat (matrix), T (hours), timestamps, coords (latitude and longitude), sensor_ids
    """
    print(f"Loading PEMS-BAY dataset: {h5_path} ...")
    ...
    print(f"Loading sensor spatial coordinates: {csv_path} ...")
    ...
    coords[~valid_mask] = np.nanmean(coords[valid_mask], axis=0)  # Fill missing values with mean
    ...
    print("🧠 Building spatial topology covariance matrices by 'hour'...")
...
def load_chbmit_eeg_topology(save_dir="data/raw"):
    """
    Automatically download and process CHB-MIT EEG data, converting it into SPD manifold covariance matrices.
    """
    os.makedirs(save_dir, exist_ok=True)
    edf_path = os.path.join(save_dir, "chb01_03.edf")

    # 1. Automatically download from MIT PhysioNet official direct link (~40MB, only needs to be downloaded once)
    url = "https://physionet.org/files/chbmit/1.0.0/chb01/chb01_03.edf?download"
    if not os.path.exists(edf_path):
        print(f"📥 Downloading raw EEG data from MIT official source (Patient 01, Session 03)...")
        urllib.request.urlretrieve(url, edf_path)
        print("✅ Download complete!")

    # 2. Read raw EDF medical file
    print("🧠 Parsing EDF EEG signals...")
    ...
    # 3. Core physical transformation: sliding window slicing -> covariance topology matrix
    window_sec = 2.0  # Calculate one topology state every 2 seconds (similar to every 1 hour for traffic)
    window_samples = int(window_sec * sfreq)
    n_windows = data.shape[1] // window_samples

    print(f"✂️ Extracting dynamic network topology using a sliding window of {window_sec} seconds...")
    cov_matrices = []
    time_centers =[]
    is_seizure =[]

    # 👨‍⚕️ Ground Truth manually annotated by doctors (Seizure time: 2996s ~ 3036s)
    seizure_start = 2996
    seizure_end = 3036
    ...
    for i in range(n_windows):
        ...
        # Calculate the 23x23 brain network functional connectivity covariance matrix
        ...
        if seizure_start <= t_center <= seizure_end:
            is_seizure.append(1)  # Seizure ongoing
        else:
